...
As long as the image is not loaded yet, a placeholder image is shown in its place.
Once the image is loaded, the placeholder image is faded away.
If the image has no placeholder, a black background is used in its place.

<Grid style={{ margin: "20px 0" }}>
  <Exhibit caption="Image that has no placeholder.">
    <div style={{ display: "grid", height: 200 }}>
      <Sample variant="no-placeholder" fill />
    </div>
  </Exhibit>
  <Exhibit caption="Placehonlder generated using SQIP.">
    <div style={{ display: "grid", height: 200 }}>
      <Sample variant="placeholder" fill />
    </div>
  </Exhibit>
